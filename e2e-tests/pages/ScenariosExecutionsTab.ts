...
export class ScenariosExecutionsTab extends BasePage {
    ...
    async addParticipantToScenariosActivity({ participantName, scenarioExecutionName, activityName }): Promise<void> {
        await this.page.getByRole('button', { name: scenarioExecutionName }).first().click();
        await this.page.getByRole('tab', { name: 'Scenarios Executions' }).click();
        await this.page.locator(`tr:has([test-data="${ activityName }"])`)
            .locator('[data-testid^="edit-activity-execution"]')
            .click();
        await this.page.getByLabel('Participants').click();
